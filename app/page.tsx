                  ...
                  >
                    <div className="flex justify-center items-start gap-2 lg:gap-4 mb-3 lg:mb-6">
                      <div className="relative flex-1 max-w-[60%]">
                        ...
                        />
                        {/* Before用のコメント */}
                        <div className={`mt-2 text-left ${isMobile ? 'text-xs' : isCenter ? 'text-sm lg:text-base' : isSide ? 'text-xs lg:text-sm' : 'text-xs'}`}>
                          <div className="text-red-600 font-semibold mb-1">Before</div>
                          <div className="text-gray-700 leading-relaxed">
                            {project.title.includes('和室') ? '古い畳と暗い照明で狭く感じる空間' :
                             project.title.includes('キッチン') ? '古い設備と収納不足で使いにくい' :
                             project.title.includes('浴室') ? '狭くて古い設備で快適性に欠ける' :
                             project.title.includes('寝室') ? '畳の部屋で現代的な生活に不向き' :
                             project.title.includes('玄関') ? '収納不足で使いにくい玄関' :
                             project.title.includes('食事') ? '低いテーブルで現代的な食事に不向き' :
                             '古い設備と使いにくいレイアウト'}
                          </div>
                        </div>
                      </div>
                      <div className="relative flex-1 max-w-[40%]">
                        ...
                        {/* After用のコメント */}
                        <div className={`mt-2 text-left ${isMobile ? 'text-xs' : isCenter ? 'text-sm lg:text-base' : isSide ? 'text-xs lg:text-sm' : 'text-xs'}`}>
                          <div className="text-teal-600 font-semibold mb-1">After</div>
                          <div className="text-gray-700 leading-relaxed">
                            {project.title.includes('和室') ? '明るいフローリングで開放的なリビングに' :
                             project.title.includes('キッチン') ? '白いキャビネットで清潔感あふれる空間に' :
                             project.title.includes('浴室') ? '現代的な設備で快適なバスルームに' :
                             project.title.includes('寝室') ? 'フローリングで快適な寝室に' :
                             project.title.includes('玄関') ? '効率的な収納で機能的な玄関に' :
                             project.title.includes('食事') ? 'ダイニングテーブルで家族団らんの空間に' :
                             '現代的な設備で快適な空間に'}
                          </div>
                        </div>
                      </div>
